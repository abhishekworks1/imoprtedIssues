...
//  ProManager
//
//  Created by Viraj Patel on 23/04/18.
...
open class AppEventBus {
    ...
    open class func postToMainThread(_ name: String, sender: AnyObject? = nil, userInfo: [AnyHashable: Any]? = nil) {
    ...
    @discardableResult
    open class func onMainThread(_ target: AnyObject, name: String, sender: AnyObject? = nil, handler: @escaping ((Notification?) -> Void)) -> NSObjectProtocol {
        return AppEventBus.on(target, name: name, sender: sender, queue: OperationQueue.main, handler: handler)
    }
    
    @discardableResult
    open class func onBackgroundThread(_ target: AnyObject, name: String, sender: AnyObject? = nil, handler: @escaping ((Notification?) -> Void)) -> NSObjectProtocol {
