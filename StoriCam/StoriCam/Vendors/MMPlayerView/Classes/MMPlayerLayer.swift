...
public class MMPlayerLayer: AVPlayerLayer {
    ...
    lazy var  bgView: UIView = {
        ...
        v.frame = .zero
        v.backgroundColor = ApplicationSettings.appClearColor
        return v
    }()
    ...
    public weak var mmIndicatorDelegate: MMPlayerLoadingProtocol?
    ...
    fileprivate func setup() {
        ...
        self.backgroundColor = ApplicationSettings.appBlackColor.cgColor
    ...
    public func replace<T: UIView>(cover:T) where T: MMPlayerCoverViewProtocol{
        ...
        cover.backgroundColor = ApplicationSettings.appClearColor
        ...
        if let k = keyPath {
            ...
            case "playbackBufferEmpty":
                if let c = change?[.newKey] as? Bool, c == true {
                    self.startLoading(isStart: true)
                    self.mmIndicatorDelegate?.playbackBufferEmpty()
                }
            case "playbackLikelyToKeepUp":
                if let c = change?[.newKey] as? Bool, c == true {
                    self.startLoading(isStart: false)
                    self.mmIndicatorDelegate?.playbackLikelyToKeepUp()
                }
            case "status":
