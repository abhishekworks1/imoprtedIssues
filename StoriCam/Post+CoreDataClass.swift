...
class InternalPostData {
    ...
    init(albumID: String?, checkedIn: String?, feelings: String?, friendExcept: String?, friendsOnly: String?, hashTags: String?, postMedia: String?, postText: String?, postType: String?, isCheckIn: Bool, previewUrlData: String?, mediaUrls: [String]?, privacy: String?, userID: String?, wallTheme: String?, thumbTime: Double?, actionType: String?, removedMedia: String?,oldPostId: String?,mentions:[String]?) {
...
class PostDataManager {
    ...
    static let shared = PostDataManager(context: (UIApplication.shared.delegate as! AppDelegate).persistentContainer.viewContext)
    ...
    func getPostUploadData(withPredicate queryPredicate: NSPredicate) -> [PostUploadData] {
        ...
        do {
            let response = try context.fetch(fetchRequest)
            return response as! [PostUploadData]
    ...
    func stopUploadFile(postData: PostData) {
        ...
        for (_,item) in getAllPostDatas().enumerated()
        {
            if postData.url == item.url {
                ...
                Utils.uploadSingleUrlStop(postData.url)
