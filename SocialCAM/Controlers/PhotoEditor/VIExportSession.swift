...
extension VIExportSession {
    ...
    fileprivate func buildDefaultVideoOutputSetting(videoTrack: AVAssetTrack) -> [String: Any] {
        ...
        let maximumSize: CGFloat = 1280
        func lowerSize() -> CGSize? {
            let width = videoTrack.naturalSize.width
            let height = videoTrack.naturalSize.height
            if width > height {
                if width > maximumSize {
                    return CGSize(width: maximumSize,
                                  height: maximumSize*height/width)
                }
            } else {
                if height > maximumSize {
                    return CGSize(width: maximumSize*width/height,
                                  height: maximumSize)
                }
            }
