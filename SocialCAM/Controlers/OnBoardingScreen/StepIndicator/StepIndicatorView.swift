...
public class StepIndicatorView: UIView {
    ...
    @IBInspectable public var finishedStep: Int = -1 {
        didSet{
            if self.annularLayers.count <= 0 {
                return
            }
            if oldValue != self.finishedStep {
                self.setStep(step: self.finishedStep, finished: true)
            }
        }
    }
    ...
    func setStep(step: Int, finished: Bool) {
        guard step < self.numberOfSteps, step >= 0 else {
            return
        }
        self.annularLayers[step].isFinished = finished
    }
