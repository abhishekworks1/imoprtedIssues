...
//  NL
//
//  Created by Jasmin Patel on 02/10/19.
//  Copyright © 2019 NextLevel. All rights reserved.
//
...
public class CircularProgress: UIView, CAAnimationDelegate {
    ...
    private var centerImageView: RoundedImageView!
    
    public var centerImage: UIImage? = UIImage() {
        didSet {
    ...
    func centerImageViewSetup() {
        self.centerImageView = RoundedImageView(frame: self.frame)
        ...
        addSubview(self.centerImageView)
        let xConst = NSLayoutConstraint(item: self.centerImageView!, attribute: .centerX, relatedBy: .equal, toItem: self, attribute: .centerX, multiplier: 1, constant: 0)
    ...
    private class CircularProgressViewLayer: CALayer {
        ...
        override func draw(in ctx: CGContext) {
            ...
            imageCtx?.addArc(center: CGPoint(x: width / 2.0, y: height / 2.0),
                             radius: arcRadius + trackLineWidth/2 - progressLineWidth/2,
