...
public protocol TrimViewDelegate: class {
...
// range
@IBDesignable public class TrimView: AVAssetTimeSelector {
    
    // MARK: - Properties
    // MARK: Color Customization
    /// The color of the main border of the view
    @IBInspectable public var mainColor: UIColor = ApplicationSettings.appClearColor {
        didSet {
            updateMainColor()
        }
    }
    ...
    @IBInspectable public var positionBarColor: UIColor = ApplicationSettings.appWhiteColor {
        didSet {
    ...
    // MARK: Interface
    public weak var delegate: TrimViewDelegate?
    ...
    let tempVideoPath = URL(fileURLWithPath: NSTemporaryDirectory()).appendingPathComponent("tmpMov.mov")
    ...
    private func setupMaskView() {
        ...
        leftMaskView.backgroundColor = ApplicationSettings.appWhiteColor
        ...
        rightMaskView.backgroundColor = ApplicationSettings.appWhiteColor
    ...
    func deleteTempFile() {
        ...
        let exist: Bool = fm.fileExists(atPath: tempVideoPath.path )
        if exist {
            try? fm.removeItem(at: tempVideoPath)
            print("file deleted")
        } else {
    ...
    func trim() {
        ...
        if compatiblePresets.contains(AVAssetExportPresetMediumQuality) {
            ...
            exportSession.outputURL = tempVideoPath
            exportSession.outputFileType = AVFileType.mp4
            let start: CMTime = CMTimeMakeWithSeconds((startTime?.seconds)!, preferredTimescale: asset.duration.timescale)
            let duration: CMTime = CMTimeMakeWithSeconds((endTime?.seconds)! - (startTime?.seconds)!, preferredTimescale: asset.duration.timescale)
            ...
            exportSession.exportAsynchronously(completionHandler: {
                ...
                default:
                    ...
                    DispatchQueue.main.async(execute: {
                        UISaveVideoAtPathToSavedPhotosAlbum(self.tempVideoPath.relativePath, self, #selector(self.video(_:didFinishSavingWithError:contextInfo:)), nil)
                    })
                }
            })
        }
    ...
    @objc func handlePanGesture(_ gestureRecognizer: UIPanGestureRecognizer) {
        ...
        case .changed:
            ...
            updateSelectedTime(stoppedMoving: false)
            self.seek(to: getTime(from: translation.x)!)
